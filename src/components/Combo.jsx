...
export function NameCombo({ value, onText, options = [], onPick, onCreate, placeholder, createLabel }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <input ref={inputRef} className="input" value={value || ''} placeholder={placeholder || t('combo_type_add')} autoComplete="off"
    ...
    <div className="relative" ref={ref}>
      ...
      {open && pos && (matches.length > 0 || (q && !exact) || catF) && (
        <div className="fixed z-50 max-h-72 overflow-auto rounded-lg border border-black/10 bg-white py-1 text-sm shadow-xl"
          ...
          {matches.map(p => (
            <button type="button" key={p.id} className="flex w-full items-start justify-between gap-3 px-3 py-2 text-left hover:bg-sand"
              onMouseDown={() => { onPick(p); setOpen(false) }}>
              <span className="min-w-0">
                <span className="block truncate">{p.name}{p.sku ? <span className="text-ink/40"> · {p.sku}</span> : null}</span>
                {p.description ? <span className="block truncate text-xs text-ink/45">{p.description}</span> : null}
              </span>
