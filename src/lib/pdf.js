...
export function documentPDF({ kind, doc: d, items, customer, company }) {
  ...
  // bill to
  y += 8
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(9); pdf.setTextColor(...MUTED)
  ...
  autoTable(pdf, {
    ...
    theme: 'striped',
    headStyles: { fillColor: MOSS, textColor: 255, fontSize: 9 },
  ...
  const row = (label, val, bold) => {
    pdf.setFont('helvetica', bold ? 'bold' : 'normal'); pdf.setFontSize(bold ? 11 : 9)
...
export function packingSlipPDF({ doc: d, items, customer, company }) {
  const pdf = new jsPDF()
  const numberLabel = d.invoice_number || d.estimate_number
  let y = header(pdf, company, 'Packing Slip', numberLabel)
  pdf.setFontSize(9); pdf.setTextColor(...MUTED)
  pdf.text(`Date: ${fmtDate(d.issue_date)}`, 196, 33, { align: 'right' })
